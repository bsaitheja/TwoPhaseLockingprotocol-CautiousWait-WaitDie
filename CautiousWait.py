...
def writeFile():
    try:
        if(output=='1'):
            f = open("header.txt")
            f1 = open(filename+"'s output - cautiouswait - Tabular Format", "w+")
            for line in f:
                f1.write(line)
        else:
            f = open("header.txt")
            f1 = open(filename+"'s output - cautiouswait - Summary Format", "w+")
            for line in f:
                f1.write(line)
        f1.write("\n")
        f1.write("\n")
        f1.write("Cautious Wait")
        f1.write("\n")
        f1.write("\n")
        return f1
    except:
    ...
    try:
        ...
        if (present_tem['state'].to_list()[0] == 'readlock') and (ongoing_process == cs_ed):
            ...
            if present_tem['state'].to_list()[0] == 'Unlocked':
                ...
                if len(ol_in_is) == 1:
                    cautios_wait(ol_in_is[0], ongoing_process, operation_value, data)
                else:
                    process_list = tran_fun.loc[tran_fun['id'] == ongoing_process,['id','ts_var','state','items','data_in_file']] 
                    
                    rnt_est = process_list['state'].to_list()[0]
                    toogle = 0
                    if len(ol_in_is) > 0:
                        for capture in ol_in_is:
                            process_list = tran_fun.loc[tran_fun['id'] == capture,['id','ts_var','state','items','data_in_file']] 						

                            cvb_stm = process_list['state'].to_list()[0]
                        ...
                        if toogle == 0:
                            file_writer.write(data+" : "+ongoing_process + ' Transaction is blocked (will resume when unlocked) \n')
...
def read_lock(operation_value, ongoing_process, data):
    try:
        ...
        if see_ms.size == 0:
            ...
            data = data.strip()
            file_writer.write( data + ' Read lock acquired by ' + ongoing_process + ' on item '+ operation_value+'\n')
            ...
            if gg_em['state'].to_list()[0] != 'writelock':
                ...
                tran_fun.loc[tran_fun['id'] == ongoing_process, ['items']] = tm_ae
                data = data.replace('\n','')
                file_writer.write( data + ' Read lock granted by ' + ongoing_process + ' on item '+ operation_value+'\n')
                
            else:
                onhold_tran = yi_og_it[0]
                requesting_transactions = ongoing_process
                cautios_wait(onhold_tran, requesting_transactions, operation_value, data)
    except:
        print("Please re-run the code, something unexpected happend :(")
...
#it checks if lock table of the item for transaction t2 is in block state then abort transaction t1 otherwise transaction t1will wait for transaction t2 to release the lock and transaction t1 is added to wait in lock table for the corresponding item
def cautios_wait(onhold_tran, requesting_transactions, gg_em, data):
    try:
        global transaction_table    
        global lock_table
        global file_writer

        # get timeStamp of the holding transaction
        process_list = tran_fun.loc[tran_fun['id'] == onhold_tran,['id','ts_var','state','items','data_in_file']] 		
        HTS = process_list['state'].to_list()[0]
        # get timeStamp of the requesting transaction
        process_list = tran_fun.loc[tran_fun['id'] == requesting_transactions,['id','ts_var','state','items','data_in_file']] 		
        requestingTimeStamp = process_list['state'].to_list()[0]
        if  'Blocked' == HTS:
            
            file_writer.write('As per Cautious Wait methodology, '+ requesting_transactions + ' transaction will abort   ' + onhold_tran+' \n')
            abort(requesting_transactions, data)
            

            
        else:
            file_writer.write(data+" : "+requesting_transactions + ' Transaction is blocked, As per Cautious Wait methodology\n')
            ...
            # add transaction to waiting list for that item
            hold_trann = ''
...
#e collect all the list of waiting transactions and check if the transaction is not in waiting then update the lock table for the corresponding item and change state to Unlock
...
def main():
    ...
    global file_writer
    try:
        with open(filename,'r') as data_in_file: 
            ts_var = 0
            for data in data_in_file.readlines():
                ...
                if tran_record.size == 0:
                    ...
                    if tran_record['state'].to_list()[0] == 'Aborted':
                        file_writer.write(data+' : Operation not executed as Transaction ' + ongoing_process + ' is aborted - ignore\n')
        ...
        if(output=='1'):
            ...
            write1()
        print("Output file create for "+filename)
    except:
